...
void Emplace()
{
    ...
    vInt.emplace_back(
        val ); // Its a vardiac template, when you want to create objects with directly inside containers.
...
int main()
{
    ...
    /*    auto it = data.find(6);
        ...
        auto found = data.contains(6); // c++ 20 introduced "contains" member that returns boolean value, after checking
       whether there is a value present or not if(found)
        {
            std::cout << "found";
        }
